...
等字眼但未附圖時觸發）：** MUST 先問使用者「有沒有實際視覺稿（Figma/截圖/設計圖）可
...
**依賴變更檢查（新增依賴需確認）：** 比對本次改動中 manifest 檔（`package.json`、
`pyproject.toml`、`Cargo.toml`、`go.mod` 等，以專案實際存在者為準）的 diff：新增一個前次
版本不存在的 top-level 套件名稱 → 視為新增依賴，MUST 讀取 `references/output-templates.md`
的「Step 3：新增依賴確認」並輸出，等待確認後才能繼續；僅既有套件版本號變化 → 視為升級，
不觸發，直接繼續。
...
MUST 在任何版本號變更前，讀取 `references/eval-scenarios.md`，逐一對照全部十一個場景（E01–E11），全部通過才能蓋新版本號。逐條列出比對結果，不得僅憑摘要宣稱「Eval 通過」（見 E08）。
